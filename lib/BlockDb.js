...
var db = imports.db || level(config.leveldb + '/blocks',{ maxOpenFiles: MAX_OPEN_FILES });
...
BlockDb.prototype._changeBlockHeight = function(hash, height, cb) {
  ...
  this.fromHashWithInfo(hash, function(err, bi) {
    ...
    console.log('ereasdasda');
    db.batch(dbScript2.concat(dbScript1), function(err, d) {
     console.log(err, d, 'haehsdh8sdfhsf');
     cb(err)
    });
  });
};
