...
TransactionDb.prototype._fromBuffer = function(buf) {
  var buf2 = digibyte.util.buffer.reverse(buf);
...
TransactionDb.prototype.getStandardizedTx = function(tx, time, isCoinBase) {
  ...
  tx.txid = tx.hash;
  var ti = 0;
  tx.vin = tx.inputs.map(function(txin) {
    var ret = {
      n: ti++
    };
    if (isCoinBase) {
  ...
  var to = 0;
  tx.vout = tx.outputs.map(function(txout) {
    var val;
    var valueSat;
    if (txout.s) {
      var s = new Script(txout.s);
      var addrs = new digibyte.Address.fromScriptPubKey(s, config.network);
      ...
      var s = new Script(txout.script);
      var addrs = s.toAddress();
      if (addrs) {
        val = {
...
TransactionDb.prototype._addScript = function(tx, relatedAddrs) {
  ...
  for (var ii in tx.vout) {
    ...
    if (o.scriptPubKey && o.scriptPubKey.addresses && o.scriptPubKey.addresses[0] && !o.scriptPubKey.addresses[1]) {
      ...
      var sat = o.valueSat || digibyte.Unit.fromDGB(o.value).toSatoshis();
