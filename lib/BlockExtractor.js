...
BlockExtractor.prototype.getNextBlock = function(cb) {
  ...
  var info = {
    header: {
      version: this.currentParser.readInt32LE(),
      prevHash: this.currentParser.read(32),
      merkleRoot: this.currentParser.read(32),
      time: this.currentParser.readUInt32LE(),
      bits: this.currentParser.readUInt32LE(),
      nonce: this.currentParser.readUInt32LE()
    },
    transactions: []
  };
  var transactions = this.currentParser.readVarintNum();
  for (var i = 0; i < transactions; i++) {
    var tx = {
      inputs: [],
      outputs: []
    };
    tx.version = this.currentParser.readInt32LE();
    var sizeTxIns = this.currentParser.readVarintNum();
    var hasWitnesses = false;
    if (sizeTxIns === 0 && this.currentParser.buf[this.currentParser.pos] !== 0) {
      this.currentParser.pos += 1;
      hasWitnesses = true;
      sizeTxIns = this.currentParser.readVarintNum();
    }
    for (var i = 0; i < sizeTxIns; i++) {
      var input = {
        prevTxId: this.currentParser.readReverse(32),
        outputIndex: this.currentParser.readUInt32LE(),
        scriptBuffer: this.currentParser.readVarLengthBuffer(),
        sequence: this.currentParser.readUInt32LE(),
      };
      tx.inputs.push(input);
    }
    var sizeTxOuts = this.currentParser.readVarintNum();
    for (var j = 0; j < sizeTxOuts; j++) {
      var output = {};
      output.satoshis = this.currentParser.readUInt64LEBN();
      var size = this.currentParser.readVarintNum();
      if (size !== 0) {
        output.script = this.currentParser.read(size);
      } else {
        output.script = new buffer.Buffer([]);
      }
    ...
    if (hasWitnesses) {
      for (var k = 0; k < sizeTxIns; k++) {
        ...
        for (var l = 0; l < itemCount; l++) {
          var size = this.currentParser.readVarintNum();
          var item = this.currentParser.read(size);
          witnesses.push(item);
        }
        tx.inputs[k].setWitnesses(witnesses);
      }
    }
    tx.nLockTime = this.currentParser.readUInt32LE();
    info.transactions.push(tx);
  }
  this.currentParser.pos += blockSize;
  console.log(info);
  var b = new Block(info)
  console.log(info, b.toObject());
  //var blockSize = this.currentParser.readUInt32LE();
  //var b = digibyte.Block.fromRawBlock(this.currentParser);
  //console.log(b.toObject(), this.currentParser)
  // = new Block(this.currentParser.buf);
  //console.log(b.toObject())
  //b.parse(this.currentParser.buf);
  //b.getHash();
  this.errorCount=0;
  return cb(null,b);
};
