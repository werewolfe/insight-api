...
var bitcore         = require('digibyte'),
    RpcClient       = require('digibyted-rpc'),
...
Rpc._parseTxResult = function(info) {
  ...
  info.vout.forEach( function(o) {
    valueOutSat += bitcore.Unit.fromDGB(o.value).toSatoshis();
  });
  info.valueOut = bitcore.Unit.fromSatoshis(valueOutSat).toDGB();
  info.size     = b.length;
  return info;
};
...
Rpc.getBlock = function(hash, cb) {
  ...
  bitcoreRpc.getBlock(hash, function(err,info) {
    ...
    if (info.result.height)
      info.result.reward =  50;//(BitcoreBlock.getBlockValue(info.result.height) / bitcore.util.COIN);
