...
PeerSync.prototype.handleInv = function(peerMessage, peer) {
  var invs = peerMessage.inventory;
...
PeerSync.prototype.handleTx = function(info) {
  var self =this;
  var tx = this.sync.txDb.getStandardizedTx(info.transaction);
...
PeerSync.prototype.handleBlock = function(info) {
  ...
  var blockHash = block.toObject().header.hash
  self.log('[p2p_sync] Handle block: %s (allowReorgs: %s)', blockHash, self.allowReorgs);

  var tx_hashes = block.transactions.map(function(tx) {
    return tx.hash;
  });
...
PeerSync.prototype.run = function() {
  ...
  this.peerdb.forEach(function(datum) {
    self.peerman._connectPeer(datum);
  });

  this.peerman.on('peerconnect', function(conn) {
    self.connected = true;
    conn.on('inv', function(message) {
      self.handleInv(message, conn)
    });
